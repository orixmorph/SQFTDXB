 ...
 */
export function mapBaserowRowToProperty(row: Record<string, any>): Property | null {
  ...
  // 17. Assigned Agent
  const agentRawName = extractText(row['Agent']);
  const agentKey = agentRawName.toLowerCase();
  const knownProfile = AGENT_PROFILES[agentKey];

  const agent: Agent = {
    id: `agent-${slugify(agentRawName || 'sahar')}`,
    name: agentRawName || 'Sahar Najahi',
    title: knownProfile?.title || 'Senior Secondary Market Specialist',
    phone: knownProfile?.phone || '+971 58 864 8093',
    whatsapp: knownProfile?.whatsapp || '971588648093',
    email: knownProfile?.email || 'sales@sqftdxb.com',
    photo: knownProfile?.photo || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=400&q=80',
    reraNumber: '',
    verifiedDeals: 140,
  };
...
export async function getLiveProperties(forceRefresh = false): Promise<{
  ...
  try {
    ...
    console.log(`[Baserow] Fetching live properties from: ${url}`);
    ...
    // Map each row and filter only LIVE properties
    const liveProps: Property[] = [];
    for (const row of rows) {
      const mapped = mapBaserowRowToProperty(row);
      if (mapped) {
...
export async function getLiveAgents(forceRefresh = false): Promise<Agent[]> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const row of rows) {
        ...
        agents.push({
          ...
          email: extractText(row['Email']) || knownProfile?.email || 'advisors@sqftdxb.com',
          photo: photos[0] || knownProfile?.photo || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=400&q=80',
      ...
      // Sort: agents with properties first (descending count), then others
      agents.sort((a, b) => {
    ...
    console.error('[Baserow] Failed to fetch agents table:', error);
  }

  // Fallback: construct agents directly from live properties and known profiles
  ...
  result.sort((a, b) => (b.propertyCount || 0) - (a.propertyCount || 0));
  return result;
}
